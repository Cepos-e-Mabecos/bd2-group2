...
def get_locaisconsumo():
    return utils.getAll(locaisconsumoColumns, f"SELECT * FROM selectlocaisconsumo();")

@locaisconsumo.route('/api/locaisconsumo/<cod_LocalConsumo>', methods=['GET'])
def get_localconsumo(cod_LocalConsumo):
    return utils.getOne(locaisconsumoColumns, f"SELECT * FROM selectlocalconsumo({cod_LocalConsumo});")

@locaisconsumo.route('/api/locaisconsumo', methods=['POST'])
def post_localconsumo():
    return utils.postOne(f"CALL insertlocaisconsumo({json.dumps(request.json)});")

@locaisconsumo.route('/api/locaisconsumo/<cod_LocalConsumo>', methods=['PUT'])
def put_localconsumo(cod_LocalConsumo):
    return utils.putOne(f"CALL updatelocaisconsumo({cod_LocalConsumo}, {json.dumps(request.json)});")
